...
<body>
    <nav>
        <a href="./admin-un.php">غير مجابة</a>
        <a class="active" href="./admin-done.php">مجابة</a>
  ...
  if (result.isConfirmed) {
    window.location.href = './admin-done.php?delete=' + id
  }
});
} 
</script>
                ...
                echo '<button class="reply-btn" data-id="' . $row['id'] . '">تعديل</button>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
    ...
    <div id="replyModal" class="modal">
        ...
        <div class="modal-content">
            ...
            <form id="replyForm" method="POST" enctype="multipart/form-data">
                <textarea id="ans" required name="answer" cols="30" rows="5" placeholder="اكتب إجابتك هنا..."></textarea>
                <input type="file" hidden name="audio" accept="audio/*">
                <input type="hidden" name="question_id" id="modalQuestionId">
                <button type="submit" name="submit">تعديل الرد</button>
...
document.getElementById("replyForm").onsubmit = function(event) {
    ...
    fetch('update_question.php', {
      ...
      .then(result => {
          ...
          window.location.href = './admin-done.php'
      })
            ...
            navigator.mediaDevices.getUserMedia({ audio: true })
                .then(stream => {
                    ...
                    mediaRecorder.onstop = () => {
                        ...
                        audioChunks = [];
                        document.quنerySelector('input[name="audio"]').files = [new File([audioBlob], 'recording.wav')];
                    };
                    mediaRecorder.start();
                });
        }
